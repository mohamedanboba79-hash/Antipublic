...
# ============ إعدادات البوت ============
BOT_TOKEN = "8937588348:AAFCpn3onbonlU_MCt6OqQxitFD-AA3kFS8"  # ضع توكن البوت هنا
WEBAPP_URL = "https://your-webapp-url.com"  # رابط الـ Web App
ADMIN_IDS = [1867486900]  # ضع معرفات الأدمن هنا (Telegram User IDs)
...
async def webapp_data_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data:
        try:
            # هنا هنستقبل البطاقات من الويب اب
            # لكننا بنستخدم طريقة الـ API عشان نرسل الملف مباشرة
...
# في دالة button_handler - تحديث زر Filter
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'clean':
        ...
        await query.edit_message_text(
            "🔍 **أرسل لي ملف البطاقات**\n\n"
            "سأقوم بفتح واجهة التصفية المتقدمة بعد استلام الملف.",
            parse_mode='Markdown'
        )
        context.user_data['action'] = 'filter'
    ...
    # ===== لوحة تحكم الأدمن =====
        ...
        today_ops = sum(1 for u in users.values() if u.get('last_activity', '').startswith(today))
